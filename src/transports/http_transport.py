...
class HTTPTransportManager:
    ...
    def __init__(self, mcp_server: Server, config: HTTPConfig):
        ...
        if not MCP_AVAILABLE:
            raise RuntimeError("MCP SDK not available. Install with: pip install mcp>=1.0.0")
    ...
    def _create_app(self) -> Starlette:
        ...
        routes = [
            ...
            Route(
                self.config.health_path, self.health_check, methods=["GET"]
            ),  # /health (deprecated)
            Route(
                self.config.ready_path, self.readiness_check, methods=["GET"]
            ),  # /ready (deprecated)
        ]
        ...
        if self.config.sse_enabled:
            routes.append(
                Route(f"{self.config.base_path}/sse", self.handle_sse_request, methods=["GET"])
            )
    ...
    async def handle_json_request(self, request: Request) -> JSONResponse:
        ...
        logger.info(
            ...
            extra={"session_id": session_id, "method": request.method, "path": request.url.path},
        )
        ...
        try:
            ...
            logger.debug("JSON request processed successfully", extra={"session_id": session_id})

            return JSONResponse(content=response_data, headers={"Mcp-Session-Id": session_id})

        except json.JSONDecodeError as e:
            logger.error(f"Invalid JSON in request: {e}", extra={"session_id": session_id})
            return JSONResponse(
                content={
                    "jsonrpc": "2.0",
            ...
            logger.error(
                f"JSON request failed: {e}", extra={"session_id": session_id}, exc_info=True
            )
            return JSONResponse(
                content={
                    "jsonrpc": "2.0",
    ...
    async def handle_sse_request(self, request: Request) -> StreamingResponse:
        ...
        logger.info("SSE request received", extra={"session_id": session_id})
        ...
        async def event_generator():
            ...
            try:
                ...
                logger.debug("SSE stream established", extra={"session_id": session_id})

            except Exception as e:
                logger.error(
                    f"SSE streaming failed: {e}", extra={"session_id": session_id}, exc_info=True
                )
    ...
    async def run(self) -> None:
        ...
        """
        if uvicorn is None:
            raise RuntimeError("Uvicorn not available. Install with: pip install uvicorn[standard]")
        ...
        logger.info(f"HTTP server ready: http://{self.config.host}:{self.config.port}")
        logger.info(f"Health check: http://{self.config.host}:{self.config.port}/health")
        logger.info(
            f"MCP endpoint: http://{self.config.host}:{self.config.port}{self.config.base_path}/messages"
        )

        if self.config.sse_enabled:
            logger.info(
                f"SSE endpoint: http://{self.config.host}:{self.config.port}{self.config.base_path}/sse"
            )
