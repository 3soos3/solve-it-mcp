...
class HTTPConfig(BaseModel):
    ...
    stateless: bool = Field(
        default=True,
        description="Use stateless session management for horizontal scaling"
    )
    json_response_default: bool = Field(
        default=True,
        description="Default to JSON responses (vs SSE streaming)"
    )
    sse_enabled: bool = Field(
        default=True,
        description="Enable SSE streaming endpoint"
    )
    
    # CORS configuration
    cors: CORSConfig = Field(
        default_factory=CORSConfig,
        description="CORS configuration for web clients"
    )
    
    # Endpoint paths
    base_path: str = Field(
        default="/mcp/v1",
        description="Base path for MCP API endpoints"
    )
    ...
    metrics_path: str = Field(
        default="/metrics",
        description="Metrics endpoint path (reserved for future use)"
    )
...
class OpenTelemetryConfig(BaseModel):
    ...
    trace_sample_rate: float = Field(
        default=1.0,
        description="Base trace sampling rate (overridden by get_sample_rate())"
    )
    metric_export_interval_ms: int = Field(
        default=60000,
        description="Metric export interval (milliseconds)"
    )
    
    # Performance tuning
    max_queue_size: int = Field(
        default=2048,
        description="Maximum span queue size before dropping"
    )
    batch_schedule_delay_ms: int = Field(
        default=5000,
        description="Delay before exporting span batches (milliseconds)"
    )
