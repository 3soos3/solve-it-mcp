...
class TestSTDIOTransport:
    ...
    async def test_stdio_transport_with_mock_server(self):
        ...
        with patch(
            "transports.stdio_transport.mcp.server.stdio.stdio_server",
            return_value=MockSTDIOServer(),
        ):
    ...
    async def test_stdio_transport_initialization_options(self):
        ...
        # Create a mock server
        mock_server = MagicMock(spec=Server)
        mock_server.get_capabilities = MagicMock(return_value={"tools": {}, "prompts": {}})
    ...
    async def test_stdio_handles_exception_group(self):
        ...
        async def mock_run_exception(*args, **kwargs):
            raise MockExceptionGroup(
                "Multiple errors", [ValueError("Error 1"), RuntimeError("Error 2")]
            )
